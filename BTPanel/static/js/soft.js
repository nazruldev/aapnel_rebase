        ...
        $.ajax({
            ...
            success: function(data) {
                if (data && typeof data.status === 'number') {
                    if (data.status !== 0) {
                        layer.msg((data.message && data.message.msg) || data.msg || 'Upload failed', { icon: 2 });
                        return;
                    }
                    if (data.message && typeof data.message === 'object') data = data.message;
                }
    ...
    input_zip: function (plugin_name,tmp_path,data) {
        bt.soft.show_speed_window({title:'Installing, this may take a few minutes...',status:true},function(){
            $.post('/plugin?action=input_zip', { plugin_name: plugin_name, tmp_path: tmp_path }, function (rdata) {
                layer.closeAll()
                var ok = rdata && (rdata.status === true || rdata.status === 0);
                var msg = (rdata && rdata.message) ? rdata.message : (rdata && rdata.msg);
                if (ok) {
                    soft.get_list();
                }
                setTimeout(function () { layer.msg(msg || '', { icon: ok ? 1 : 2 }) }, 1000);
            });
        });
    }
};
